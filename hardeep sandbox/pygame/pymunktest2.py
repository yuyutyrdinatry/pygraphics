# From: http://misc.slowchop.com/misc/browser/muckaround/pymunk/hungry1.py
import pygame
from pygame.locals import *
from pygame.color import *
import pymunk as pm
from pymunk.vec2d import *
import math
import random

X,Y = 0,1
### Physics collision types
COLLTYPE_DEFAULT = 0
COLLTYPE_MOUSE = 1

space = None
balls = []

def flipy(y):
    """Small hack to convert chipmunk physics to pygame coordinates"""
    return -y+600

def mouse_coll_func(s1, s2, cs, normal_coef, data):
    """Simple callback that increases the radius of circles touching the mouse"""
    return False
    if s2.collision_type != COLLTYPE_MOUSE and type(s2) == pm.Circle:
        s2.radius += 0.15
    return False

def is_hungry(s1):
    return s1.hunger > 10

def is_dead_hungry(s1):
    return s1.hunger > 1000

def default_coll_func(s1, s2, cs, normal_coef, data):
    if type(s1) != pm.Circle:
        return True
    if type(s2) != pm.Circle:
        return True
    if s1.mass < s2.mass:
        s2, s1 = s1, s2

    # is s1 hungry?
    if not is_hungry(s1):
        return True

    # cant eat if too small
    if s2.radius < 2:
        s2.radius = 2
        return True
    if s2.mass < 1:
        s2.mass = 1
        return True

    mass_taken = s2.radius * 0.5
    radius_taken = s2.radius * 0.01

    s1.mass += mass_taken * 1.1
    s1.radius += radius_taken * 1.1
    s2.mass -= mass_taken
    s2.radius -= radius_taken

    s1.hunger = 0

    return True

def main():

    pygame.init()
    screen = pygame.display.set_mode((1024, 768))
    clock = pygame.time.Clock()
    running = True

    ### Physics stuff
    pm.init_pymunk()
    global space
    space = pm.Space()
    space.gravity = Vec2d(0.0, -900.0)

    space.resize_static_hash()
    space.resize_active_hash()

    ## Balls
    global balls
    balls = []

    ### Mouse
    mouse_body = pm.Body(1e100, 1e100)
    mouse_shape = pm.Circle(mouse_body, 3, Vec2d(0,0))
    mouse_shape.collision_type = COLLTYPE_MOUSE
    space.add(mouse_body, mouse_shape)

    space.add_collisionpair_func(COLLTYPE_MOUSE, COLLTYPE_DEFAULT, mouse_coll_func, ("hello", "world"))

    space.set_default_collisionpair_func(default_coll_func, ('hello'))

    ### Static line
    line_point1 = None
    static_lines = []
    run_physics = True

    while running:
        for event in pygame.event.get():
            if event.type == QUIT:
                running = False
            elif event.type == KEYDOWN and event.key == K_ESCAPE:
                running = False
                space.add(body, shape)
                balls.append(shape)

            elif event.type == MOUSEBUTTONDOWN and event.button == 3:
                if line_point1 is None:
                    line_point1 = Vec2d(event.pos[X], flipy(event.pos[Y]))
            elif event.type == MOUSEBUTTONUP and event.button == 3:
                if line_point1 is not None:

                    line_point2 = Vec2d(event.pos[X], flipy(event.pos[Y]))
                    print line_point1, line_point2
                    body = pm.Body(1e100, 1e100)
                    shape= pm.Segment(body, line_point1, line_point2, 5.0)
                    shape.friction = 0.99
                    shape.elasticity = 1
                    space.add_static(shape)
                    static_lines.append(shape)
                    line_point1 = None

            elif event.type == KEYDOWN and event.key == K_SPACE:
                run_physics = not run_physics

        p = pygame.mouse.get_pos()
        mouse_pos = Vec2d(p[X],flipy(p[Y]))
        mouse_body.position = mouse_pos

        def makeball(m):
            body = pm.Body(10, 10)
            body.position = m
            # shape = pm.Circle(body, 1 + 10 * random.random(), Vec2d(0,0))
            shape = pm.Circle(body, 1 + 5 * random.random(), Vec2d(0,0))
            shape.friction = random.random() * 10
            shape.mass = shape.radius * random.random()
            shape.elasticity = random.random()
            # shape.hunger_rate = random.random()
            shape.hunger_rate = 1
            shape.hunger = 0.
            space.add(body, shape)
            balls.append(shape)

        if pygame.mouse.get_pressed()[0]:
            makeball(mouse_pos)

        if len(balls) < 100:
            makeball(Vec2d(random.random() * 1024, random.random() * 500))

        ### Update physics
        if run_physics:
            goodballs = []
            for ball in balls:
                badball = 0

                body = ball.body

                ball.hunger += ball.hunger_rate * ball.radius

                if is_dead_hungry(ball):
                    ball.radius *= 0.99

                if is_dead_hungry(ball) and ball.radius < 5:
                    print 'ball starved'
                    badball = 1

                elif body.position.x < 0:
                    badball = 1
                elif body.position.x > 1500:
                    badball = 1
                elif body.position.y < -600:
                    badball = 1
                elif body.position.y > 600:
                    badball = 1

                if badball:
                    space.remove(ball, ball.body)
                else:
                    goodballs.append(ball)

            balls = goodballs

            dt = 1.0/60.0
            for x in range(1):
                space.step(dt)

        ### Draw stuff
        screen.fill(THECOLORS["white"])

        # Display some text
        font = pygame.font.Font(None, 16)
        text = """LMB: Create ball
LMB + Shift: Create many balls
RMB: Drag to create wall, release to finish
Space: Pause physics simulation"""
        y = 5
        for line in text.splitlines():
            text = font.render(line, 1,THECOLORS["black"])
            screen.blit(text, (5,y))
            y += 10

        for ball in balls:
            r = ball.radius
            v = ball.body.position
            rot = ball.body.rotation_vector
            p = int(v.x), int(flipy(v.y))
            p2 = Vec2d(rot.x, -rot.y) * r * 0.9
            red = 255. / 1000 * ball.hunger
            if red > 255:
                red = 255
            g = 255. / 10 * ball.hunger
            if g > 255:
                g = 255
            g = 0

            pygame.draw.circle(screen, (red, g, 0), p, int(r), 1)
            pygame.draw.line(screen, THECOLORS["red"], p, p+p2)

        if line_point1 is not None:
            p1 = line_point1.x, flipy(line_point1.y)
            p2 = mouse_pos.x, flipy(mouse_pos.y)
            pygame.draw.lines(screen, THECOLORS["black"], False, [p1,p2])

        for line in static_lines:
            body = line.body

            pv1 = body.position + line.a.rotated(math.degrees(body.angle))
            pv2 = body.position + line.b.rotated(math.degrees(body.angle))
            p1 = pv1.x, flipy(pv1.y)
            p2 = pv2.x, flipy(pv2.y)
            pygame.draw.lines(screen, THECOLORS["lightgray"], False, [p1,p2])

        ### Flip screen
        pygame.display.flip()
#        clock.tick(50)
        pygame.display.set_caption("fps: " + str(clock.get_fps()))

def remove_ball(ball):
    global space
    global balls


if __name__ == '__main__':
#    doprof = 0
#    if not doprof:
    main()
#    else:
#        import cProfile, pstats
#
#        prof = cProfile.run("main()", "profile.prof")
#        stats = pstats.Stats("profile.prof")
#        stats.strip_dirs()
#        stats.sort_stats('cumulative', 'time', 'calls')
#        stats.print_stats(30)